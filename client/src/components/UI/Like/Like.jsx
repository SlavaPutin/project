...
const Like = ({like, setLike, id, props}) => {
    const [active, setActive] = useState(false)
    const [counter, setCounter] = useState(like)
    const svglike =[cl.likesvg]
    const Activate = () => {
        if (active === true){
             setCounter(counter-1)
             setLike(counter-1, id)
             setActive(false)
        } else
            {   setCounter(counter+1)
                setLike(counter+1, id)
    ...
    if (active==true){
            svglike.push(cl.likesvgactive)
    } 
    

    return(
        <div className={cl.wraplike}>
        <button {...props} className={cl.like} onClick={Activate} >
            <svg   width="24" height="24" viewBox="0 0 24 24"  className={svglike.join(' ')} xmlns="http://www.w3.org/2000/svg">
<path d="M19.2373 6.23731C20.7839 7.78395 20.8432 10.2727 19.3718 11.8911L11.9995 20.0001L4.62812 11.8911C3.15679 10.2727 3.21605 7.7839 4.76269 6.23726C6.48961 4.51034 9.33372 4.66814 10.8594 6.5752L12 8.00045L13.1396 6.57504C14.6653 4.66798 17.5104 4.51039 19.2373 6.23731Z" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
        ...
        <h5 className={cl.counter}>{counter}</h5>
        </div>
    )
}
